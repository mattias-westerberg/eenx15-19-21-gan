...
class NordhGenerator(Generator):
    def __init__(self, image_size):
        ...
        with tf.variable_scope("generator") as scope:
            ...
            #self.model.add(tf.keras.layers.LeakyReLU(alpha=0.2))
            self.model.add(tf.keras.layers.Conv2D(128, (3, 3), strides=(2, 2), padding='same', input_shape=(image_size, image_size, 3)))
            """ 32x32x128 """
            self.model.add(tf.keras.layers.BatchNormalization())
            self.model.add(tf.keras.layers.LeakyReLU(alpha=0.2))
            self.model.add(tf.keras.layers.Conv2D(256, (3, 3), strides=(2, 2), padding='same'))

            """ 16x16x256 """
            self.model.add(tf.keras.layers.BatchNormalization())
            self.model.add(tf.keras.layers.LeakyReLU(alpha=0.2))
            self.model.add(tf.keras.layers.Conv2DTranspose(128, (2, 2), strides=(2, 2), padding='same'))
            """ 32x32x128 """
            self.model.add(tf.keras.layers.BatchNormalization())
            self.model.add(tf.keras.layers.LeakyReLU(alpha=0.2))
            self.model.add(tf.keras.layers.Conv2DTranspose(128, (3, 3), strides=(2, 2), padding='same'))
